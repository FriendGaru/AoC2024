...
class SecurityRoom:
    ...
    def get_quads(self):
        ...
        return tl_quad_count, tr_quad_count, br_quad_count, bl_quad_count

    def is_symmetrical(self):
        tl_quad_count, tr_quad_count, br_quad_count, bl_quad_count = self.get_quads()
        if tl_quad_count == tr_quad_count and bl_quad_count == br_quad_count:
            return True
        else:
            return False

    def has_conseq(self, conseq_count):
        conseq_count = 0
        coord_dict = self.build_coord_robot_dict()
        for y in range(self.room_height):
            conseq_count = 0
            for x in range(self.room_width):
