            ...
            for possible_route in possible_routes:
                ...
                for direction in possible_route:
                    if (my_loc_label, super_bot_start_label, direction) in self.best_direction_dict:
                        possible_topmost_path_len += self.best_direction_dict[(my_loc_label, super_bot_start_label, direction)]
                    else:
                        topmost_path_add_len = self.super_bot.demand_directions(direction, super_bot_start_label)
                        possible_topmost_path_len += topmost_path_add_len
                        self.best_direction_dict[my_loc_label, super_bot_start_label, direction] = topmost_path_add_len
                    super_bot_start_label = direction
                    my_loc_label = self.keypad.get_new_label_from_dir(my_loc_label, direction)
                if best_topmost_path_len is None or possible_topmost_path_len < best_topmost_path_len:
                    best_topmost_path_len = possible_topmost_path_len


        self.best_routes_dict[(start_label, end_label)] = best_topmost_path_len
        return best_topmost_path_len
...
class CongaLine:
    ...
    def solve(self, code):
        ...
        for digit in code:
            final_path_add_len = self.k_robot.demand_directions(digit, bottom_bot_loc_label)
            final_path_len += final_path_add_len
    ...
    def solve_all_codes(self, code_list, verbose=True):
        ...
        for code in code_list:
            assert len(code) == 4
            shortest_path_len = self.solve(code)
            code_complexity = shortest_path_len * int(code[:3])
            if verbose:
                print(f"{code}: path {shortest_path_len} * {code[:3]}")
